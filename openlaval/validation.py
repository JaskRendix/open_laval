...
def validate_blade_geometry(x: np.ndarray, lower: np.ndarray, upper: np.ndarray) -> list[str]:
    ...
    if min_thickness <= 0:
        ...
        warnings.warn(f"Warning: Very small minimum thickness detected ({min_thickness:.4f}).")
    ...
    # Ensure upper and lower surfaces don't cross unexpectedly
...
def validate_config_limits(cfg) -> list[str]:
    ...
    if cfg.mach_in <= 1.0:
        warnings.warn(f"Warning: Inlet Mach number ({cfg.mach_in}) is subsonic. OpenLaval is optimized for supersonic blades.")
